...
class Reload extends Command {
  ...
  async run(message, args, level) { // eslint-disable-line no-unused-vars  
    ...
    if (!command) {
      const event = this.client.events.get(args[0]);
      if (!event) return message.reply({ content: `${args[0]} is neither a command, a command alias, or an event. Please check your spelling and try again.`, allowedMentions: { repliedUser: (replying === "true") }});
